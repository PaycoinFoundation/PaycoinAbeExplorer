...
Run the Paycoin client to ensure that your copy of the block chain is
up to date.

Choose or create a system account to run Paycoin-Abe.  Replace USER with its
...
Perform the initial data load:
    ...
    python -m Paycoin-Abe.abe --config abe-pg.conf --commit-bytes 100000 --no-serve
...
Then run the web server as:
    ...
    python -m Paycoin-Abe.abe --config abe-pg.conf
